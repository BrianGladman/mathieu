...
the radial functions for spheroidal wave functions. The use of
the size parameter (c) instead of the traditional (q) is consistent
with usage for spheroidal functions (see Handbook of Mathematical
Functions). So is the use of xi = cosh(z) as the radial coordinate.
...
Math. 65, 1-23 (2007). A pdf version of this article is available
for download on the website indicated above. [Note that the argument
for both cos and sin in the two equations in (10) is in error and
should be r times theta.]

5. Accuracy of Results Using 64-bit Arithmetic
----------------------------------------------
...
6. Accuracy of Results Using Real*16 Arithmetic
...
Mathieu provides optional file output of results for inspection
and diagnostics under the control of three settings in the
parameter file at the start of the source code:

    output ::   mathieu the returns function values it computes
                to the calling program but if this parameter
                is set to 'true' they will also be output to
                the files fort.20 (radial functions) and
                fort.30 (angular functions)

    debug ::    if this parameter is set detailed debugging is
                output to fort.40 (radial) and fort.50 (angular)

    warn ::     this parameter enables warnings if the program
                detects reduced precision in results or lack of
                convergence in internnal calculations
