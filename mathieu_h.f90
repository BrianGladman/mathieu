...
module mathieu
    ...
    subroutine matfcn(lnum, ioprad, izxi, icq, isq, qc, r, iopang, narg, arg, &
        ...
        real(knd), intent (out) ::  mc1c(lnum), mc1dc(lnum), mc23c(lnum), mc23dc(lnum), &
                                    ...
                                    ce(lnum, narg), ced(lnum, narg), se(lnum, narg), sed(lnum, narg)

        integer     ndec, nex, kindd, kindq, minacc, max, maxd, maxj, maxk, maxn, maxp, &
                    maxlp, maxbp, maxkbp, ndm, ismax, maxterm, ngau, j, jarg, limbes, limj, &
                    limbpe, numlp, ibflag, iopbpe, iopneu, iopbp1, iopbes, limjt, iopr, kflag, &
                    kflaga1, kflagb1, m1flag, jbesa, jbesb, jbesmax, jbp1a, jbp1b, jbp1, jneua, &
                    jneub, jbpe, jbpemax, jneumax, iss1, iss2, iflag, jmfb, jsubb, igauss, ja, &
                    limsc, jbpeb, li, l, ix, istop, naccrs_, naccrc_, issd, limdrad, limdang, jang, &
                    limd, limmf, jmf, limdneu, limndel, ienra, ia01, ienrb, jmfa, jsuba, &
                    jbesa1, jbesb1, jbp1a1, jbp1b1, numi, jsubmax, jtes, limr1, iopcs, nsubc1, &
        ...
        ! Here is where the user sets kindd, the number of bytes available in double
        ! precision data for the computer that coblfcn is run on. Similarly kindq,
        ! the number of bytes available in quadruple precision data, is set here.
        ! This allows coblfcn to be run on computers with values for kindd and kindq
        ! different than 8 and 16, respectively.
        kindd = 8
        kindq = 16
        ...
        if (ioprad /= 0 .and. isq /= 1 .and. x1 /= 0.0e0_knd) then
            ...
            ndm = ndec - minacc
            if(ndm < 2  .or. (ndm < 4 .and. cm >  5.0e0_knd) &
                        .or. (ndm < 6 .and. cm > 10.0e0_knd) &
                        .or. (ndm < 8 .and. cm > 15.0e0_knd) &
                        .or. cm > 20e0_knd) then
                maxk = 1.5 * cm + 100
        ...
        allocate (eiga(lnum), eigb(lnum))
        allocate (ce_(narg), ced_(narg), se_(narg), sed_(narg), naccc(narg), naccs(narg))
        allocate (ibese(maxlp), ibese1(maxlp), ibese2(maxlp),ineue(maxlp), ineue2(maxlp), ineue3(maxlp))
        allocate (enra(maxd), blista(maxd), glista(maxd), enrb(maxd), blistb(maxd), glistb(maxd))
        allocate (cbesf(maxj), cbesf1(maxj), cbesf2(maxj), cbesdf(maxj), cbesdf1(maxj), &
                  cbesdf2(maxj), cbesdr(maxj), cbesdr1(maxj), cbesdr2(maxj))
        allocate (cbesn(maxlp), cbesn1(maxlp), cbesn2(maxlp), cneun(maxlp), cneun2(maxlp), cneun3(maxlp))
        ...
        ! set up the array for the angle values
        if(iopang /= 0) then
            do jarg = 1, narg
        ...
        if (ioprad /= 0) then
            if(izxi == 1) then
                tstr = "(1x, ' z = ', " // kstr // ")"
                if (output) write(20, tstr) z
                if (debug)  write(40, tstr) z
            else if (izxi == 2) then
                tstr = "(1x, 'xi = ', " // kstr // ")"
                if (output) write(20, tstr) xi
                if (debug)  write(40, tstr) xi
            end if

            if(icq == 1) then
                tstr = "(1x, ' q = ', " // kstr // ")"
                if (output) write(20, tstr) q
                if (debug)  write(30, tstr) q
            end if

            if(icq == 2) then
                if(isq == 1) tstr = "(1x, ' c = ', " // kstr // ")"
                if (isq == -1) tstr = "(1x, ' c = i times', " // kstr // ")"
                if (output) write(20, tstr) cm
                if (debug)  write(40, tstr) cm
            end if
        endif
        ...
        if (iopang /= 0) then
            if(icq == 1) then
                tstr = "(1x, ' q = ', " // kstr // ")"
                if (output) write(30, tstr) q
                if (debug)  write(50, tstr) q
            else if(icq == 2) then
                if(isq == 1) tstr = "(1x, ' c = ', " // kstr // ")"
                if (isq == -1) tstr = "(1x, ' c = i times', " // kstr // ")"
                if (output) write(30, tstr) cm
                if (debug)  write(50, tstr) cm
            end if
        end if
        ...
        if(isq /= 1) then
            iopneu = 1
            ndm = ndec - minacc
            if(ndm >=  4 .and. cm <=  5.0e0_knd) iopneu = 0
            if(ndm >=  6 .and. cm <= 10.0e0_knd) iopneu = 0
            if(ndm >=  8 .and. cm <= 15.0e0_knd) iopneu = 0
            if(ndm >= 10 .and. cm <= 20.0e0_knd) iopneu = 0
            if(iopneu /= 0) then
                iopbpe = 0
                iopbes = 0
                iopbp1 = 1
        ...
        if(ioprad /= 0) then
            ...
            if(isq == 1 .and. x1 /= 0.0e0_knd) call cbesj(cm, xb, limj, maxj, &
              ...
              if(isq /= 1) then
                  ismax = int(0.85e0_knd * cm)
                  if(x1 < 1.0e0_knd) ismax = ismax * 1.5
                  if(x1 < 0.1e0_knd .and. x1 >= 0.00001e0_knd) ismax = ismax * int(3.0e0_knd ** (-1.0e0_knd - log10(x1)))
                  if(x1 < 0.00001e0_knd) ismax = ismax * 81
                  limbpe = max(limbpe, ismax + int(cm / pi) + 4 * ndec + int(cm) + 102)
                  numlp = max(numlp, ismax + int(cm / pi) + 2)
              end if
              limjt = max(limbes, limbpe)
              iopr = 1
        ...
        jbpeb = 0
        if (debug) then
            if(ioprad /= 0 .and. isq == 1 .and. x1 == 0.0q0) write(40, &
                 "(1x,/,5x,'Note that when x = 1.0, some of the radial', &
            ...
            if(ioprad /= 0 .and. isq == -1 .and. x1 == 0.0q0) write(40, &
                 "(1x,/,5x,'When x = 1.0 and c is not small and l is', &
        ...
        l_loop: do li = 1, lnum
            l = li - 1
            if (output) then
                if(iopang /= 0) write(30, "(1x,'l = ',i6)") l
            end if
            ix = l - 2 * (l / 2)
            rl = real(l, knd)
            istop = 0
            ...
            if(iopbpe /= 0 .and. l > 1) limdrad = max(limdrad, jbpemax + jbpemax + 40 + int(cm / 25))
            ...
            limd = max(limd, limmf)
            if(isq /= 1 .and. ioprad /= 0 .and. iopneu /= 0) then
                if(iopneu /= 3 .or. l <= 1) then
                    limdneu = 1.5 * cm + 100
                    if(x1 < 1.0e0_knd .and. x1 >= 0.01e0_knd) limdneu = 40 / x1 + cm / sqrt(x1) + 1.5 * cm + 200
                    if(x1 < 0.01e0_knd .and. x1 > 0.0e0_knd) limdneu = 34 / x1 + 1.5 * cm + 1.4 * cm / sqrt(x1) + 200
                end if
                if(iopneu == 3 .and. l > 1) limdneu = jneumax + jneumax + limndel + 100
                limdneu = min(limdneu, maxk)
                if(iopneu /= 0) limd = max(limd, limdneu)
            end if
            if(limd > 2 * maxd) limd = 2 * maxd - 5
            ...
            call convera(l, cm, limd, eiga1, eiga3, eiga4, eiga5, ndec, maxd, &
                          enra, ienra, sgna, a01, ia01, blista, glista, eigaval)
            if(l == 1) ienrb = (3 * ndec + int(cm)) / 2
            if(l /= 0) call geteigb(l, cm, eigb1, eigb2, eigb3, eigb4, eigb5, eigbval)
            ...
            if (debug) then
                  tstr = "(1x,'l =',i5, 5x,'eigenvalue a =', " // kstr // ")"
                  if(ioprad /= 0 .and. l == 0) write(40, tstr) l, esa
                  tstr = "(1x,'l =',i5, 5x,'eigenvalue a =', " // kstr // "/, 14x, 'eigenvalue b =', " // kstr // ")"
                  if(ioprad /= 0 .and. l /= 0) write(40, tstr) l, esa, esb
                  if(iopang /= 0 .and. l /= 0) write(50, tstr) l, esa, esb
                  tstr = "(1x,'l =',i5, 5x,'eigenvalue a =', " // kstr // ")"
                  if(iopang /= 0 .and. l == 0) write(50, tstr) l, esa
            end if

            r_code: if(ioprad /= 0) then
                ! determine Mathieu radial functions

                call dnorma(l, limd, maxd, ndec, enra, ce0, cepio2, cedpio2, jmfa, jsuba)
                if(l /= 0) call dnormb(l, limd, maxd, ndec, enrb, sed0, sepio2, sedpio2, jmfb, jsubb)
                jmf = max(jmfa, jmfb)
                jsubmax = max(jsuba, jsubb)
                jtest = ndec - jsubmax
                if(jtest < 0) jtest = 0
                if(isq /= 1 .and. kflag /= 1) then
                    if(jtest >= minacc .or. jsubmax <= 1) then
                        kflag = 1
                        iopbpe = 1
                        iopbp1 = 0
                        iopneu = 0
                ...
                if(l > 0 .and. eigaval <= eigavalp) exit
                if(l > 1 .and. eigbval <= eigbvalp) exit
                eigavalp = eigaval
                ...
                r1_code: if(iopbes /= 0) then
                ...
                else if(isq /= 1 .or. iopbp1 /= 0) then
                    ! calculation of m1 using a series of products of cylindrical
                    ! Bessel functions of the first kind (q < 0, c imaginary)
                    if (debug) then
                        tstr = "(1x,'m1 calculation using Bessel product expansion')"
                        write(40, tstr)
                    end if
                    if(l == 0 .and. x1 /= 0.0e0_knd) then
                    ...
                    if(x1 == 0.0e0_knd) call m1bpe0(iopcs, iss1, ismax, l, cm, &
                                             ...
                                             b12, ib12, jbp1a, nsubc1, mc1c_, mc1e_, &
                                             mc1dc_, mc1de_)
                    if(x1 /= 0.0e0_knd) call m1bpe(iopcs, iss1, ismax, l, cm, x1, &
                                             ...
                                             ia01, b12, ib12, jbp1a, nsubc1, mc1c_, &
                                             mc1e_, mc1dc_, mc1de_)
                    ...
                    if(l /= 0) then
                        ...
                        if(x1 == 0.0e0_knd) call m1bpe0(iopcs, iss2, ismax, l, cm, &
                                                 ...
                                                 maxj, maxlp, nex, cbesf1, cbesn1, &
                                                 ibese1, cbesdf1, cbesdr1, a01, ia01, &
                                                 b12, ib12, jbp1b, nsubs1, ms1c_, ms1e_, &
                                                 ms1dc_, ms1de_)
                        if(x1 /= 0.0e0_knd) call m1bpe(iopcs, iss2, ismax, l, cm, x1, &
                                                 ...
                                                 ibese1, cbesdf1, cbesdr1, cbesf2, &
                                                 cbesn2, ibese2, cbesdf2, cbesdr2, a01, &
                                                 ia01, b12, ib12, jbp1b, nsubs1, ms1c_, &
                                                 ms1e_, ms1dc_, ms1de_)
                ...
                if (debug) then
                    ...
                    if((isq == 1 .or. (isq == -1 .and. ix == 0)) .and. l /= 0) write(40, tstr) ms1c_, ms1e_, ms1dc_, ms1de_
                    tstr = "(10x,'mc1 = i times '," // fstr // ", i8, 5x, 'mc1d = i times '," // fstr // ", i8)"
                    if(isq == -1 .and. ix == 1) write(40, tstr) mc1c_, mc1e_, mc1dc_, mc1de_
                    tstr = "(10x,'ms1 = i times '," // fstr // ", i8, 5x, 'ms1d = i times '," // fstr // ", i8)"
                    if(isq == -1 .and. ix == 1 .and. l /= 0) write(40, tstr) ms1c_, ms1e_, ms1dc_, ms1de_
                end if

                r23_code: if(iopbpe /= 0) then
                    ! determine Mathieu radial functions of the second kind (when q is positive
                    ! or Mathieu radial functions of the third kind when q is negative)
                    ...
                    if (debug) then
                        ...
                        if(isq == -1) write(40, "(1x,'m3 calculation using Bessel product series')")
                    end if
                    limbpe = lnum + 4 * ndec + int(cm) + 100
                    if(iopbpe <= 1) then
                            xb = 0.5e0_knd * (xi + sqrt(x1 * (x1 + 2.0e0_knd)))
                            ...
                            limbes = 2 * int(cm * xb) + 2 * ndec
                            numlp = lnum
                            if(igauss /= 1 .and. isq == -1) then
                                if((cm * xb > 1.0e0_knd) .and. (cm * xb < 40.0e0_knd)) then
                                    call gauss (ngau, ndec, xr, wr)
                                    igauss = 1
                                end if
                            end if
                            if(isq == 1) call cneuy (cm, xb, limbpe, maxn, numlp, maxlp, ndec, &
                    ...
                    if(isq /= -1) then
                        call m2bpe (iopcs, iss1, l, cm, x1, limbpea, ndec, maxd, enra, enrb, &
                                    ...
                                    cneudr2, a01, ia01, jbpea, nsubbpec, mc2c_, mc2e_, &
                                    mc2dc_, mc2de_)
                        wroncc = (mc1c_ * mc2dc_) * 10.0e0_knd ** (mc1e_+mc2de_)
                        if(x1 /= 0.0e0_knd) wroncc = wroncc - (mc2c_ * mc1dc_) * 10.0e0_knd ** (mc2e_+mc1de_)
                        naccrc_ = -int(log10(abs((wroncc - 2.0e0_knd / pi) / (2.0e0_knd / pi)) + dec))
                        if(naccrc_ < 0) naccrc_ = 0
                        naccrc_ = min(naccrc_, ndec - 1)
                    else
                        call m3bpe (iopcs, l, cm, x1, limbpea, ndec, maxd, enra, enrb, &
                                ...
                                mc3c_, mc3e_, mc3dc_, mc3de_)
                        wroncc = mc1c_ * mc3dc_ * 10.0e0_knd ** (mc1e_+mc3de_)
                        if(x1 /= 0.0e0_knd) wroncc = wroncc- &
                                                mc3c_ * mc1dc_ * 10.0e0_knd ** (mc3e_+mc1de_)
                        naccrc_ = -int(log10(abs((wroncc - (2.0e0_knd / pi))/ &
                                (2.0e0_knd / pi)) + dec))
                        if(naccrc_ < 0) naccrc_ = 0
                        naccrc_ = min(naccrc_, ndec - 1)
                        if(x1 == 0.0e0_knd) then
                            if(naccrc_ == 0) mc3c_ = 0
                            if(naccrc_ == 0) mc3e_ = 0
                            if(naccrc_ < minacc) mc3dc_ = (2.0e0_knd / pi) / mc1c_
                            iterm = int(log10(abs(mc3dc_)))
                            mc3de_ = -mc1e_ + iterm
                            mc3dc_ = mc3dc_ * 10.0e0_knd ** (-iterm)
                            if(abs(mc3dc_) < 1.0e0_knd) then
                                mc3dc_ = mc3dc_ * 10.0e0_knd
                                mc3de_ = mc3de_ - 1
                            end if
                        end if
                    end if
                    ...
                    if(l /= 0) then
                        ...
                        if(isq /= -1) then
                            call m2bpe (iopcs, iss2, l, cm, x1, limbpeb, ndec, maxd, enra, enrb, &
                                        maxj, maxlp, maxn, nex, cbesf1, cbesn1, ibese1, &
                                        cbesdf1, cbesdr1, cneuf2, cneun2, ineue2, cneudf2, &
                                        cneudr2, b12, ib12, jbpeb, nsubbpes, ms2c_, ms2e_, &
                                        ms2dc_, ms2de_)
                            wroncs = -ms2c_ * ms1dc_ * 10.0e0_knd ** (ms2e_+ms1de_)
                            if(x1 /= 0.0e0_knd) wroncs = wroncs + ms1c_ * ms2dc_ * 10.0e0_knd ** (ms1e_+ms2de_)
                            naccrs_ = -int(log10(abs((wroncs - (2.0e0_knd / pi)) / (2.0e0_knd / pi)) + dec))
                            if(naccrs_ < 0) naccrs_ = 0
                            naccrs_ = min(naccrs_, ndec - 1)
                        else
                            call m3bpe (iopcs, l, cm, x1, limbpeb, ndec, maxd, enra, enrb, &
                                    maxj, maxlp, maxkbp, nex, cbesf1, cbesn1, ibese1, &
                                    cbesdf1, cbesdr1, cneuf3, cneun3, ineue3, cneudf3, &
                                    cneudr3, a01, ia01, b12, ib12, pi, jbpeb, nsubs4, &
                                    ms3c_, ms3e_, ms3dc_, ms3de_)
                            wroncs = -ms3c_ * ms1dc_ * 10.0e0_knd ** (ms3e_+ms1de_)
                            if(x1 /= 0.0e0_knd) wroncs = wroncs + ms1c_ * ms3dc_ * 10.0e0_knd ** (ms1e_+ms3de_)
                            naccrs_ = -int(log10(abs((wroncs - (2.0e0_knd / pi)) / (2.0e0_knd / pi)) + dec))
                            if(naccrs_ < 0) naccrs_ = 0
                            naccrs_ = min(naccrs_, ndec - 1)
                            if(x1 /= 0.0e0_knd) then
                                if(naccrs_ == 0) ms3dc_ = 0.0e0_knd
                                if(naccrs_ == 0) ms3de_ = 0
                                if(naccrs_ < minacc) ms3c_ = - (2.0e0_knd / pi) / ms1dc_
                                iterm = int(log10(abs(ms3c_)))
                                ms3e_ = -ms1de_ + iterm
                                ms3c_ = ms3c_ * 10.0e0_knd ** (-iterm)
                                if(abs(ms3c_) < 1.0e0_knd) then
                                    ms3c_ = ms3c_ * 10.0e0_knd
                                    ms3e_ = ms3e_ - 1
                                end if
                            end if
                        end if
                    ...
                    if (debug) then
                        if(isq /= -1) then
                            tstr = "(15x,'Wronskian accuracy =',i3, ' decimal digits.'/,10x,'mc2 = ', " &
                                // fstr // ", i6, 5x,'mc2d = '," // fstr // ", i6)"
                            write(40, tstr) naccrc_, mc2c_, mc2e_, mc2dc_, mc2de_
                            tstr = "(15x,'Wronskian accuracy =',i3, ' decimal digits.'/,10x,'ms2 = ', " &
                                // fstr // ", i6, 5x,'ms2d = '," // fstr // ", i6)"
                            if(l > 0) write(40, tstr) naccrs_, ms2c_, ms2e_, ms2dc_, ms2de_
                        else
                            tstr = "(15x,'Wronskian accuracy =',i3,' decimal digits.'/,10x,'mc3 = ', " &
                                // fstr // ", i8, 5x,'mc3d = '," // fstr // ", i8)"
                            if(ix == 1) write(40, tstr) naccrc_, mc3c_, mc3e_, mc3dc_, mc3de_
                            tstr = "(15x,'Wronskian accuracy =',i3,decimal digits.'/,10x,'ms3 = ', " &
                                // fstr // ", i8, 5x,'ms3d = '," // fstr // ", i8)"
                            if(ix == 1 .and. l > 0) write(40, tstr) naccrs_, ms3c_, ms3e_, ms3dc_, ms3de_
                            tstr = "(15x,'Wronskian accuracy =',i3, ' decimal digits.'/,10x,'mc3 = i times '," &
                                // fstr // ", i8, 5x,'mc3d = i times '," // fstr // ", i8)"
                            if(ix == 0) write(40, tstr) naccrc_, mc3c_, mc3e_, mc3dc_, mc3de_
                            tstr = "(15x,'Wronskian accuracy =',i3,' decimal digits.'/,10x,'ms3 = i times '," &
                                // fstr // ", i8,5x,'ms3d = i times '," // fstr // ", i8)"
                            if(l > 0 .and. ix == 0) write(40, tstr) naccrs_, ms3c_, ms3e_, ms3dc_, ms3de_
                        end if
                    end if
                    ...
                    if(isq /= 1) then
                        ...
                        if(iopneu /= 0) then
                            ...
                            if(ibflag /= 0) then
                                limn = 1.5 * cm + 100
                                if(x1 < 1.0e0_knd .and. x1 >= 0.01e0_knd) limn = 40 / x1 + cm / sqrt(x1) + 1.5 * cm + 200
                                if(x1 < 0.01e0_knd .and. x1 > 0.0e0_knd) limn = 34 / x1 + 1.5 * cm + 1.4 * cm / sqrt(x1) + 200
                                ...
                                call cneuk(cm, xb, limn, maxk, numlp, maxlp, ndec, nex, limbes, pi, &
                                           gamma, ngau, wr, xr, iopr, cneuf, cneun, ineue, cneudf, &
                                           cneudr)
                                ibflag = 0
                            end if
                            if(iopneu == 1) iopneu = 2

                            if(iopneu /= 4) then

                                if(iopneu /= 3 .or. l <= 1) then
                                    limneua = 1.5 * cm + 100
                                    if(x1 < 1.0e0_knd .and. x1 >= 0.01e0_knd) limneua = 40 / x1 + cm / sqrt(x1) + 1.5 * cm + 200
                                    if(x1 < 0.01e0_knd .and. x1 > 0.0e0_knd) limneua = 34 / x1 + 1.5 * cm + 1.4 * cm / sqrt(x1) + 200
                                end if
                                if(iopneu == 3 .and. l > 1) limneua = jneumax + jneumax + limndel + 100
                                limneua = min(limneua, limn - 2)
                                iopcs = 1
                                if(ix == 0) m3bot = cepio2
                                if(ix == 1) m3bot = sepio2
                                if (debug) then
                                    write(40, "(1x,'m3 calculation using Bessel function series')")
                                end if
                                call m3neu (iopcs, l, cm, x1, limneua, ndec, minacc, maxd, &
                                        enra, enrb, maxk, maxlp, nex, cneuf, cneun, ineue, &
                                        cneudf, cneudr, m3bot, pi, jneua, ndig, mc3c_, mc3e_, &
                                        mc3dc_, mc3de_, jtermflag)
                                wroncc = mc1c_ * mc3dc_ * 10.0e0_knd ** (mc1e_+mc3de_) - mc3c_ * mc1dc_ * 10.0e0_knd ** (mc3e_+mc1de_)
                                naccrc_ = -int(log10(abs((wroncc - (2.0e0_knd / pi)) / (2.0e0_knd / pi)) + dec))
                                if(naccrc_ < 0) naccrc_ = 0
                                naccrc_ = min(naccrc_, ndec - 1)
                                if(l /= 0) then
                                    ...
                                    if(l > 1 .and. iopneu == 3) limneub = jneumax + jneumax + limndel + 100
                                    ...
                                    if(l > 0) call m3neu (iopcs, l, cm, x1, limneub, ndec, &
                                                           ...
                                                           cneudr, m3bot, pi, jneub, ndig, ms3c_, &
                                                           ms3e_, ms3dc_, ms3de_, jtermflag)
                                    wroncs = ms1c_ * ms3dc_ * 10.0e0_knd ** (ms1e_ + ms3de_) - ms3c_ * ms1dc_ * 10.0e0_knd ** (ms3e_+ms1de_)
                                    naccrs_ = -int(log10(abs((wroncs - (2.0e0_knd / pi)) / (2.0e0_knd / pi)) + dec))
                                    if(naccrs_ < 0) naccrs_ = 0
                                    naccrs_ = min(naccrs_, ndec - 1)
                                    if(jtermflag == 1) jneub = jneub + 2 * limndel
                                end if
                                jneumax = max(jneua, jneub, jneumax, limn - 2)
                                iopneu = 3
                                if (debug) then

                                    write(40, "(15x,'Wronskian accuracy =',i3,' decimal digits.')") naccrc_

                                    tstr = "(10x,'mc3 = i times '," // fstr // ", i8, 5x,'mc3d = i times '," // fstr // ", i8)"
                                    if(ix == 0) write(40, tstr) mc3c_, mc3e_, mc3dc_, mc3de_

                                    tstr = "(10x,'mc3 = ', " // fstr // ", i8, 5x,'mc3d = '," // fstr // ", i8)"
                                    if(ix == 1) write(40, tstr) mc3c_, mc3e_, mc3dc_, mc3de_

                                    if(l > 0) write(40, "(15x,'Wronskian accuracy =',i3,' decimal digits.')") naccrs_

                                    tstr = "(10x,'ms3 = i times '," // fstr // ", i8, 5x,'ms3d = i times '," // fstr // ", i8)"
                                    if(ix == 0 .and. l > 0) write(40, tstr) ms3c_, ms3e_, ms3dc_, ms3de_

                                    tstr = "(10x,'ms3 = ', " // fstr // ", i8, 5x,'ms3d = '," // fstr // ", i8)"
                                    if(ix == 1 .and. l > 0) write(40, tstr) ms3c_, ms3e_, ms3dc_, ms3de_
                                end if
                            end if
                        end if
                    end if
                ...
                if(iopneu == 4) iopneu = 1
                if (output) then

                    if(isq /= -1) then

                        tstr = "(1x, i5, 2x, 4(" // fstr // ", i6, 2x), i2)"
                        write(20, tstr) l, mc1c_, mc1e_, mc1dc_, mc1de_, mc2c_, mc2e_, mc2dc_, mc2de_, naccrc_

                        tstr = "(8x, 4(" // fstr // ", i6, 2x), i2)"
                        if(l > 0) write(20, tstr) ms1c_, ms1e_, ms1dc_, ms1de_, ms2c_, ms2e_, ms2dc_, ms2de_, naccrs_
                    else
                        tstr = "(1x, i5, 2x, 4(" // fstr // ", i7, 2x), i2)"
                        write(20, tstr) l, mc1c_, mc1e_, mc1dc_, mc1de_, mc3c_, mc3e_, mc3dc_, mc3de_, naccrc_
                        tstr = "(8x, 4(" // fstr // ", i7, 2x), i2)"
                        if(l > 0) write(20, tstr) ms1c_, ms1e_, ms1dc_, ms1de_, ms3c_, ms3e_, ms3dc_, ms3de_, naccrs_
                    end if
                end if

                if (warn) then
                    if(ioprad == 1 .and. naccrc_ < 6) then
                        write(60,*) ' est. acc. = ',naccrc_, ' digits for xi = ', xi,' cm, q = ',cm, q,' l = ',l
                    end if
                    if(ioprad == 1 .and. l /= 0 .and. naccrs_ < 6) then
                        write(60,*) ' est. acc. = ',naccrs_, ' digits for xi = ', xi,' cm, q = ',cm, q,' l = ',l
                    end if
                end if
                mc1c(li) = mc1c_
                mc1dc(li) = mc1dc_
        ...
        deallocate (enra, blista, glista, enrb, blistb, glistb)
        deallocate (ibese, ibese1, ibese2,ineue, ineue2, ineue3)
        deallocate (ce_, ced_, se_, sed_, naccc, naccs)
        deallocate (eiga, eigb)
...
!  real(knd) scalars and vectors
        ...
        integer ibese1(maxlp), ibese(maxlp)
...
!  real(knd) scalars and vectors
        ...
        integer   ibese(maxlp), ineue(maxlp)
!
!  convergence ratio dcon is set according to the number of decimal
!  digits available
        dcon = 10.0e0_knd ** (-ndec - 2)
